<div class="relative {{ $class ?? '' }}" x-data="{ orderingText: '{{ isset($value) ? $value : $name }}', open: false,  }" wire:ignore.self>
    @isset($label)
    ...
    <div class="absolute flex flex-col gap-16px border-2 border-[#000] rounded-xl w-full p-16px mt-16px bg-[#fff] z-10 max-h-[200px] overflow-y-scroll" x-show="open">                        
        ...
        <div class="cursor-pointer relative" @click="orderingText = '{{ $item[$showingValue][$showingValueSecond] }}'; open = false">
            <input type="radio" wire:model="{{ $inputName ?? '' }}" value="{{ $item[$inputValue][$inputValueSecond] }}" class="absolute h-full w-full opacity-0 cursor-pointer" @if(isset($value) && $value == $item[$showingValue][$showingValueSecond]) checked @endif>
