<div class="relative min-w-[200px] {{ $class ?? '' }}" x-data="{ orderingText: '{{ isset($value) ? $value : $name }}', open: false,  }">
    ...
    <div class="absolute flex flex-col gap-16px border-2 border-[#000] rounded-xl w-full p-16px mt-16px bg-[#fff] z-10 max-h-[200px] overflow-y-scroll" x-show="open" x-cloak>                        
        ...
        <div class="cursor-pointer relative" @click="orderingText = '{{ $item }}'; open = false">
            <input type="radio" value="{{ $item }}" name="{{ $inputName ?? '' }}" wire:model="{{ $inputName ?? '' }}" class="absolute h-full w-full opacity-0 cursor-pointer" @if(isset($value) && $value == $item) checked @endif> 
